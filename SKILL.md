...
```
1. 直接打开博主个人主页检测访问权限（Cookie + xsec 双重验证）
2. 能提取到 xsec_token → 继续；否则 → 打开登录页等扫码
3. 登录后回主页验证 xsec 可用
4. 增量滚动 + 逐屏提取笔记链接（scrollBy 非 scrollTo）
5. 逐个打开笔记页面，用 fetch(img.src) 下载全部图片
6. 保存到指定目录（断点续传，已下载自动跳过）
```

### 登录检测策略

- 不与 `window.__INITIAL_STATE__` 较劲（经常未填充，误报"未登录"）
- 优先用 Cookie：`a1` + `web_session` 同时存在
- 终极验证：开主页提取 > 0 个 xsec_token = 有效
- 登录页每 3s 检测一次，检测到后立即回主页确认
